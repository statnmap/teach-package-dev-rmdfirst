...
- What is going on when building a package using {fusen}?
...
---
class: slide 
### [Short] 1: Create a new (classical) project
...
knitr::include_graphics("images/fusen_new_project.png")
```
...
---
class: slide 
### [Short] 2: Add the {fusen} Rmd template

- Create the RMarkdown template of your package using {fusen}: "dev_history.Rmd"
  + Run this line in the Console
```{r, eval=FALSE}
fusen::add_dev_history(name = "teaching")
```

--

- Here are the main parts of a package, in a unique Rmd file
...
- Restart your RStudio session
  + File > Recent Projects > mytools (same as current project)
  + Or `rstudioapi::openProject()`
...
---
class: slide
### Exercise: Can you draw ?

- Draw a schematic view of the links between "dev_history.Rmd" content and package structure
- Explore the multiple files and folders created
- Can you spot where went each part of the "dev_history.Rmd" ?
  + Some were copied in multiple places
  + Some were copied nowhere

```{r, echo=FALSE, out.width="40%"}
knitr::include_graphics("images/fusen_skeleton.png")
```

> You can take a picture and post it in the Chat room
...
> Procedure is split in two parts to let you practice.
...
knitr::include_graphics("images/fusen_start_hello.png")
```

> You could also directly run: `usethis::create_project(path = "~/hello")`
...
- Create the RMarkdown template of your package using {fusen}: "dev_history.Rmd"
  + Use template name `minimal` to get empty named chunks
...
- We modify the code of the `function` chunk as we go along
...
- because we are in a Rmd, you won't be able to get the auto-completion for roxygen
...
- You "dev_history.Rmd" file should look like this:
...
> If there are any errors or warnings, read them carrefuly, address them in the "dev_history.Rmd" and inflate the package again.
...
What is the correct order for documentation to be correclty associate to files and folders?
...
B. Overview and use of the package for users  
...
> Store this commands in a `development` chunk
...
- Restart your RStudio session
  + File > Recent Projects > hello (same as current project)
  + Or `rstudioapi::openProject()`
  + The 'Build' tab should appear in RStudio
...
<br/>
<br/>
> If you are motivated, you can start again from the beginning of the 10 steps procedure
...
- Create a new "dev_history.Rmd" using `fusen::add_dev_history(name = "additional")` for a new family of functions

---
class: slide

### Present the documentation in a website
...
### What about classical way of developing a package?

- You would have to fill the different files yourself
  - "R/"
  - "tests/"
  - "vignettes/"

- While developing you could
  - Run an example of your function directly with Ctrl + Enter
...
> Be careful, when using {fusen}, if you want to modify some code, go back to the "dev_history.Rmd" and do `inflate()` again

---
class: slide

### The package is stored on two places

.pull-left[
**Development directory**

- Where you chose to create your project
- All files and directory for development
- "data-raw/", "inst/" included
- Called with `pkgload::load_all()`

]
.pull-right[
**Installed directory**

- Along with other R packages
- You do not choose its place
- Only files and directories not in '.RBuildignore'
- Compiled code if required
- Documentation compiled and included
- Updated with `devtools::install()`
- Called with `library(hello)`
]

> There maybe mismatch between code in development and installed package 
...
Store and execute these functions in a `development` chunk
...
- Add a version tracking file
  + `usethis::use_news_md()`
  + In accordance with `usethis::use_version()` to bump version number
...
- We'll speak about data in the next chapter
